...
#result, data = conn.uid('search', None, '(SINCE "1-Apr-2016")')
#result, data = conn.uid('search', None, '(SINCE "01-Apr-2014")')
result, data = conn.uid('search', None, '(SINCE "09-Sep-2015")')
...
importantWords = ['Wallace','Watson','Alison','Kyle','Mandy','Amanda','Lexi','Alexa','Sandi','Lawrence','Larry','Roger','Margaret','Amy','Jason','Josh','Jenny','Birdwell','Nanny','wife','brother','sister','mother','mom','dad','aunt','uncle','grandmother','Love','Wedding','Birthday','Child','Family','Professor','Adjunct','Manager','Developer','Salary','Position','Brown','Mazda']
...
try:
    raw_file = open('../A4/raw-email-recFix2.tsv', 'w')
...
for i in range(0, len(data)):
    ...
    if date_tuple:
        ...
        dateFmt = dt.strftime('%Y-%m-%d %H:%M:%S')
        #print dt
        #print dateFmt
        day = dt.strftime('%a')
        hour = dt.strftime('%H')
        ...
        if part.get_content_type() == "text/plain":
            ...
            for word in importantWords:
                if word in body:
                    ...
                    #print importantWords[count2], ' = ', word, body.count(word)
                count2 += 1
